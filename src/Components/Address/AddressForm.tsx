...
import { useInputStr2, useInputNumMaxLen } from '../../Hooks/useInput';
...
`;
export default ({addressFormProp}:{addressFormProp:AddressFormProp})=>{
    const {name,zip, setzip, address, setaddress,
        address2,
        phone1,
        phone2,
        phone3,
        delivermessage}=addressFormProp;
    const [modalVisible, setmodalVisible] = useState(false);
    const clickZipcodeBtn = (e:React.MouseEvent<HTMLButtonElement, MouseEvent>)=>{

        findAddress(setzip, setaddress);
    }
    const setAddressValueToInput = (adress:string)=>{
        setaddress(adress);
    }
    ...
    return(
        <Container>
            ...
            <Column>
                ...
                <AddressSearchBox>
                    <AddressSearchBoxColumn>
                        ...
                        <button type="button" onClick={clickZipcodeBtn}>우편번호</button>
                    </AddressSearchBoxColumn>
                    <AddressSearchBoxColumn>
            ...
            {modalVisible && <AddressSearchModal
                setmodalVisible={setmodalVisible}
                setAddressValueToInput={setAddressValueToInput}
            />}
        </Container>
    )
}